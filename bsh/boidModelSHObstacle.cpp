...
BoidModelSHObstacle::BoidModelSHObstacle(CLHelper* clHlpr, std::vector<Vec4> pos, std::vector<Vec4> vel,
	...
	: BoidModelGridBase(clHlpr, simP, "Boid Model SH Obstacle", true)
{
	...
	programBoid    = loadBoidProgram(kernel_path + "boidModelSHObstacle_kernel_v1.cl");
...
void BoidModelSHObstacle::createAndLoadObstacleSH(const std::vector<Vec4>& cor, const std::vector<unsigned int>& start,
...
void BoidModelSHObstacle::simulate(float dt){
	...
	//reorder pos/vel/goal into the buffer set that is not rendered this frame
	try
	{
		if (counter){
