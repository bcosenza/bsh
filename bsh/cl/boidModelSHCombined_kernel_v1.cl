	...
	float dt)
{
	...
	for (uint i = 0; i < (simParams->numBodies / lSize); i++){
		...
		for (int j = 0; j < lSize; j++){
			...
			float8 SHOtherX = (1.f / (dist + 0.01f)) * sh_eval_localX[j];
			float8 SHOtherY = (1.f / (dist + 0.01f)) * sh_eval_localY[j];
			float8 SHOtherZ = (1.f / (dist + 0.01f)) * sh_eval_localZ[j];

			float sumAllSHX = (1.f / (dist + 0.01f)) * sh_c0_localX[j] * 0.2820947917738781f * M_PI_F;
			float sumAllSHY = (1.f / (dist + 0.01f)) * sh_c0_localY[j] * 0.2820947917738781f * M_PI_F;
			float sumAllSHZ = (1.f / (dist + 0.01f)) * sh_c0_localZ[j] * 0.2820947917738781f * M_PI_F;
