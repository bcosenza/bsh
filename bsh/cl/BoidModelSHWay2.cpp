...
BoidModelSHWay2::BoidModelSHWay2(CLHelper* clHlpr, std::vector<Vec4> pos, std::vector<Vec4> vel,
	std::vector<Vec4> goal, std::vector<Vec4> color, simParams_t* simP)
	: BoidModelGridBase(clHlpr, simP, "Boid Model SH Way 2", true)
{
	...
	programBoid    = loadBoidProgram(kernel_path + "boidModelSHWay2_kernel_v1.cl");
...
void BoidModelSHWay2::simulate(float dt){
	...
	//long range avoidance evaluating the SH of every other boid
	try
	{
		if (counter){
		...
		err = kernel_useSH.setArg(19, cl::Local(sizeof(cl_float4)*(LOCAL_PREF)));
		err = kernel_useSH.setArg(20, cl::Local(sizeof(cl_float4)*(LOCAL_PREF)));
