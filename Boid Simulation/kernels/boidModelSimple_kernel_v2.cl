/*
	As boidModelSimple_kernel_v1 with fast built in vector operations.
*/
...
__kernel void boidKernel(__global float4* pos, __global float4* pos_out, __global float4* vel, __global float4* vel_out, float dt, __constant simParams_t* simParams)
{
	...
	for(int j = 0; j < simParams->numBodies; j++){
	
		//skip self
		if(j != id){
			otherBoid = pos[j];			//position of other boid
			dist = otherBoid - p;		//distance vector to other boid
			dist.w = 0.0f;

			distLength = fast_length(dist);	//length of distance vector
			
			if(distLength < 5.f){										//check if other boid is local neighborhood
				float dotP = dot(-v, dist);								
				float lenV = length(v);
				angle = dotP / (lenV * distLength);					//calculate acute angle between other boid and self (0° to 90°)
				
				if((dotP < 0.f) || (fabs(acospi(angle) * 180) > 45)){		//check if angle is greaten than 45°, because acute angle is used negative dot product indicates > 90°
					cohesion += otherBoid;									
					alignment += vel[j];	
	...
	v = v * simParams->wOwn + (cohesion * simParams->wCohesion + alignment * simParams->wAlignment + separation * simParams->wSeparation);
	v.w = 0.0f;
	...
	//using the magnitude of the velocity is nicer than clamping the speed
	float len = length(v);
